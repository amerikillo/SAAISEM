                            ...
                            <tbody>
                                <%
                                    try {
                                        con.conectar();
                                        try {
                                            ResultSet rset = con.consulta("SELECT F.F_ClaDoc,F.F_ClaCli,U.F_NomCli,DATE_FORMAT(F.F_FecApl,'%d/%m/%Y') AS F_FecApl,SUM(F.F_Monto) AS F_Costo,DATE_FORMAT(F.F_FecEnt,'%d/%m/%Y') AS F_FecEnt FROM tb_transferencias F INNER JOIN tb_uniatn U ON F.F_ClaCli=U.F_ClaCli GROUP BY F.F_ClaDoc ORDER BY F.F_ClaDoc+0;");
                                            while (rset.next()) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-info btn-block" href="generaAbastoCSV.jsp?F_ClaDoc=<%=rset.getString(1)%>"><span class="glyphicon glyphicon-download"></span></a>
                                    </td>
                                    <!--td>
